...
unsigned long ip_ct_tcp_timeout_established =   26 MINS;
#else
...
unsigned long ip_ct_tcp_timeout_time_wait =     2 SECS;
		      ...
		      enum ip_conntrack_info ctinfo)
{
	...
	if (!test_bit(IPS_SEEN_REPLY_BIT, &conntrack->status)) {
		...
		if (star9100_shnat_hook_ready) {
			star9100_shnat_nf_add_hnatable_hook(conntrack, iph, 1);
		}
#endif
	}
...
#if 0
		printk("TCP_CONNTRACK_SYN_SENT \n");
		printk("%s:th[0]: ",__FUNCTION__);
		DUMP_TUPLE2(&conntrack->tuplehash[0].tuple);
		printk("%s:th[1]: ",__FUNCTION__);
		DUMP_TUPLE2(&conntrack->tuplehash[1].tuple);
#endif
		/* SYN packet */
