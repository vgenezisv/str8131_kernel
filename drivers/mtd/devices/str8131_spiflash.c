...
static char module_name[] = "STR8131 SPI";
...
static int 
str8131_spiflash_erase(struct mtd_info *mtd, struct erase_info *instr)
{
...
static void
str8131_debug_test(loff_t from, size_t len, size_t len1, size_t len2)
{
	printk("[KC_DEBUG] sizeof(loff_t): %d\n", sizeof(loff_t));
	printk("[KC_DEBUG] str8131_debug_test() from:0x%llx from:0x%x\n", from, from);
}

static int 
str8131_spiflash_read(struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen, u_char *buf)
{
	str8131_debug_test(from, len, len, len);
...
static int 
str8131_spiflash_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf)
{
	struct spi_flash_info *flash_info = mtd->priv;
	u32 prog_size_left;
	u32 prog_size;
	...
	*retlen = 0;
	...
	while (prog_size_left) {
		...
		if (!Spi_Flash_Page_Program_Data_Bytes(0, to + prog_size_total, buf + prog_size_total, prog_size)) {
			goto write_error;
		}
...
/* ......................................................................... */
static int __devinit str8131_spiflash_init(void)
{
	struct mtd_info *mtd;
	...
	Spi_Flash_Initialize(0);
   	...
   	printk ("STR8131 SPI: Probing for Serial flash ...\n");
	if (spiflash_probe() != 0) {
		return -ENODEV;
	}
	...
	mtd->erase = str8131_spiflash_erase;
	mtd->read = str8131_spiflash_read;
	mtd->write = str8131_spiflash_write;
	mtd->priv = flash_info;
...
module_init (str8131_spiflash_init);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Star Semi Corp");
MODULE_DESCRIPTION("STR8131 SPI Flash MTD driver");
