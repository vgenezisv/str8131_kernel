...
static irqreturn_t str8131_ext_irq_handler(int this_irq, void *dev_id, struct pt_regs *regs)
{
...
static int __init str8131_inthandler_init(void)
{
	int ret;
if(debug) printk("%s: \n",__FUNCTION__);
...
#define register_ext_int(_i){\
			...
			str8131_set_interrupt_trigger (INTC_EXT_INT##_i##_BIT_INDEX,INTC_LEVEL_TRIGGER,INTC_ACTIVE_LOW);\
			if ((ret=request_irq(INTC_EXT_INT##_i##_BIT_INDEX, str8131_ext_irq_handler, 0, "testing", NULL))){\
...
/*	HAL_MISC_ENABLE_EXT_INT30_PINS();
	str8131_set_interrupt_trigger (INTC_EXT_INT30_BIT_INDEX,INTC_IRQ_INTERRUPT,INTC_LEVEL_TRIGGER,INTC_ACTIVE_LOW);
	if ((ret=request_irq(INTC_EXT_INT30_BIT_INDEX, str8131_pm_irq_handler, 0, "testing", NULL))){
...
static void __exit str8131_inthandler_exit(void) 
{ 
//#ifdef DEBUG
		if(debug) printk("%s: \n",__FUNCTION__);
//#endif
...
module_init(str8131_inthandler_init);
module_exit(str8131_inthandler_exit);
