...
#define STR8131_WATCHDOG_DATE       "20070521"
#define STR8131_WATCHDOG_VERSION    "1.0.1"
...
static void watchdog_str8131_hwinit(void)
{ 
...
static int __init str8131_watchdog_init(void)
{
    int retval;	
    
    if (machine_is_netwinder())
		return -ENODEV;
    str8131_set_interrupt_trigger (INTC_WATCHDOG_TIMER_BIT_INDEX,INTC_IRQ_INTERRUPT,INTC_EDGE_TRIGGER,INTC_RISING_EDGE);
    retval = misc_register(&watchdog_miscdev);
    if (retval < 0) return retval;
    printk(KERN_INFO "str8131_wdt.o: watchdog module version %s\n", STR8131_WATCHDOG_VERSION);
    return 0;
}

static void __exit str8131_watchdog_exit(void) 
{ 
    misc_deregister(&watchdog_miscdev);	
}

module_init(str8131_watchdog_init);
module_exit(str8131_watchdog_exit);
