...
#define STR8131_RTC_DATE       "20060628"
#define STR8131_RTC_VERSION    "1.0.0"
...
static DECLARE_WAIT_QUEUE_HEAD(str8131_rtc_wait);
...
static irqreturn_t rtc_fire(int irq, void *dev_id, struct pt_regs *regs)
{
    unsigned int volatile    rtc_int_status;	
...
static void rtc_str8131_hwinit(int ctrl, int hour, int min, int sec)
{    	
...
static ssize_t rtc_read(struct file * file, char *buf, size_t count, loff_t * ppos)
{
    ...
    add_wait_queue(&str8131_rtc_wait, &wait);
...
out:
    ...
    remove_wait_queue(&str8131_rtc_wait, &wait);
    return retval;
}
...
static int __init str8131_rtc_init(void)
{
    int error;
    ...
    printk(KERN_INFO "str8131_rtc.o: rtc module version %s\n", STR8131_RTC_VERSION);
    rtc_str8131_hwinit(0,0,0,0);
    HAL_RTC_ENABLE();
...
static void __exit str8131_rtc_exit (void)
{
    char buf[64];
...
module_init(str8131_rtc_init);
module_exit(str8131_rtc_exit);
EXPORT_NO_SYMBOLS;
