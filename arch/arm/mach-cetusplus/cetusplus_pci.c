...
u32 cetusplus_pci_irqs[4] = {0, INTC_PCI_INTA_BIT_INDEX, INTC_PCI_INTB_BIT_INDEX, 0};

static int cetusplus_pci_read_config(struct pci_bus *bus,
...
static int cetusplus_pci_write_config(struct pci_bus *bus,
	unsigned int devfn, int where, int size, u32 val)
{
	u32 v;
	u32 shift;
...
static struct pci_ops cetusplus_pci_ops = {
	.read	= cetusplus_pci_read_config,
	.write	= cetusplus_pci_write_config,
};

static struct resource cetusplus_pci_io = {
	.name	= "PCI I/O space",
...
static struct resource cetusplus_pci_nprefetch_mem = {
...
static struct resource cetusplus_pci_prefetch_mem = {
...
static int __init cetusplus_pci_setup_resources(struct resource **resource)
{
	int ret = -1;

	ret = request_resource(&iomem_resource, &cetusplus_pci_io);
	if (ret) {
		printk(KERN_ERR "PCI: unable to allocate I/O "
		       "memory region (%d)\n", ret);
		goto out;
	}
	ret = request_resource(&iomem_resource, &cetusplus_pci_nprefetch_mem);
	if (ret) {
		printk(KERN_ERR "PCI: unable to allocate non-prefetchable "
		       "memory region (%d)\n", ret);
		goto release_io;
	}
	ret = request_resource(&iomem_resource, &cetusplus_pci_prefetch_mem);
	if (ret) {
	 ...
	 */
	resource[0] = &cetusplus_pci_io;
	resource[1] = &cetusplus_pci_nprefetch_mem;
	resource[2] = &cetusplus_pci_prefetch_mem;
...
release_nprefetch_mem:
	release_resource(&cetusplus_pci_nprefetch_mem);
release_io:
	release_resource(&cetusplus_pci_io);
out:
	return ret;
}
...
int __init cetusplus_pci_setup(int nr, struct pci_sys_data *sys)
{
	if (nr != 0) {
		return 0;
	}

	if (cetusplus_pci_setup_resources(sys->resource)) {
		BUG();
	}
...
struct pci_bus *cetusplus_pci_scan_bus(int nr, struct pci_sys_data *sys)
{
	return pci_scan_bus(sys->busnr, &cetusplus_pci_ops, sys);
}

void __init cetusplus_pci_preinit(void)
{
...
void __init cetusplus_pci_postinit(void)
{
 ...
 */
static int __init cetusplus_pci_map_irq(struct pci_dev *dev, u8 slot, u8 pin)
{
	int irq;
	 ...
	 * 3      1     0
	 */
	irq = cetusplus_pci_irqs[((slot + pin - 1) & 3)];
...
static struct hw_pci cetusplus_pci __initdata = {
	.swizzle		= pci_std_swizzle,
	.map_irq		= cetusplus_pci_map_irq,
	.nr_controllers		= 1,
	.setup			= cetusplus_pci_setup,
	.scan			= cetusplus_pci_scan_bus,
	.preinit		= cetusplus_pci_preinit,
	.postinit		= cetusplus_pci_postinit,
};

static int __init cetusplus_pci_init(void)
{
	pci_common_init(&cetusplus_pci);
	return 0;
}

subsys_initcall(cetusplus_pci_init);
