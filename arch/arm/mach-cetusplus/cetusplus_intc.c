 ...
 */
static inline void cetusplus_set_irq_mode(unsigned int irq, unsigned int mode)
{
 ...
 */
static inline void cetusplus_set_irq_level(unsigned int irq, unsigned int level)
{
	volatile unsigned int val;
 ...
 */
void cetusplus_set_interrupt_trigger(unsigned int irq, unsigned int mode, unsigned int level)
{
	cetusplus_set_irq_mode(irq, mode);
	cetusplus_set_irq_level(irq, level);
}
EXPORT_SYMBOL(cetusplus_set_interrupt_trigger);

/*
 * Mask/Disable this interrupt source
 */
void cetusplus_mask_irq(unsigned int irq)
{
	// Mask/Disable this interrupt source
 ...
 */
void cetusplus_unmask_irq(unsigned int irq)
{
...
struct irqchip cetusplus_irqchip = {
	.ack	= cetusplus_mask_irq,
	.mask	= cetusplus_mask_irq,
	.unmask	= cetusplus_unmask_irq,
};

void __init cetusplus_init_irq(void)
{
	int i;
	...
	for (i = 0; i < NR_IRQS; i++) {
		if (intc_trigger_table[i].mode != INTC_TRIGGER_UNKNOWN) {
			cetusplus_set_irq_mode(i, intc_trigger_table[i].mode);
			cetusplus_set_irq_level(i, intc_trigger_table[i].level);
		}
	}
	
	for (i = 0; i < NR_IRQS;  i++) {
		set_irq_chip(i, &cetusplus_irqchip);
