...
int hsdma_init(void)
{
	...
	retval = request_irq(INTC_HSDMAC_BIT_INDEX, &hsdma_isr, SA_INTERRUPT, "STR8131 HSDMA", &hsdma_dev);
	if (retval) {
...
static int __init str8131_hsdma_init(void)
{
	int retval;

	printk("STR8131 HSDMA driver init\n");
	retval = hsdma_init();
#ifdef STR8131_HSDMA_TEST
	if (retval == 0) {
		static int str8131_hsdma_test(void);
		(void)str8131_hsdma_test();
	}
#endif
	return retval;
}

static void __exit str8131_hsdma_exit(void)
{
	        return;
}

module_init(str8131_hsdma_init);
module_exit(str8131_hsdma_exit);
...
static void hsdma_copy_end(hsdma_xfer_t *hsdma_xfer, int err)
{
	...
	printk("STR8131 HSDMA Testing end\n");

	if (err) {
		printk("STR8131 HSDMA Testing failed!!\n");
	} else {
		for (i = 0; i < NUM_HSDMA_VEC; i++) {
			if (memcmp((src + i * NUM_BYTES_PER_VEC), (dst + i * NUM_BYTES_PER_VEC), NUM_BYTES_PER_VEC) == 0) {
				printk("STR8131 HSDMA Testing success started at offset: %04d:\n", i * NUM_BYTES_PER_VEC);
			} else {
				int j;
				u8 *psrc;
				u8 *pdst;
				...
				printk("STR8131 HSDMA Testing error started at offset: %04d\n", i * NUM_BYTES_PER_VEC);
